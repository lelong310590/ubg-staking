...
import { Contract, ContractMain, ContractToken, ESMCStatus, Ethereum, Transaction } from "./smc.types";
...
export class SmcService {
  ...
  static async initialize(store: Store): Promise<any> {
    ...
    try {
      ...
      // Contracts
      const contractMAIN = new this.web3.eth.Contract(this.configs['SMC_MAIN_ABI'], this.configs['SMC_MAIN_ADDRESS']);
      const contractUSDT = new this.web3.eth.Contract(this.configs['SMC_USDT_ABI'], this.configs['SMC_USDT_ADDRESS']);

      this.contractIDO = new this.web3.eth.Contract(this.configs['SMC_IDO_ABI'], this.configs['SMC_IDO_ADDRESS'], { from: this.address }) as any;
      this.contractIDOToken = new this.web3.eth.Contract(this.configs['SMC_IDO_TOKEN_ABI'], this.configs['SMC_IDO_TOKEN_ADDRESS'], { from: this.address }) as any;

      this.contractMAIN = {
        poolInfo: (pid) => contractMAIN.methods.poolInfo(pid)
          .call({ from: this.address }),
        userPoolInfo: async (pid) => {
          const pendingReward = await contractMAIN.methods.pendingReward(pid, this.address).call();
          return contractMAIN.methods.userInfo(pid, this.address)
            .call()
            .then((res: any) => ({
        ...
        deposit: (pid, amountUSDT) => contractMAIN.methods.deposit(pid, NumberUtils.cryptoConvertToStringAmount(amountUSDT, 6), this.getRefCode())
          .send({ from: this.address }),
        claimReward: (pid) => contractMAIN.methods.claimReward(pid)
          .send({ from: this.address })
      };

      this.contractUSDT = {
        balanceOf: () => contractUSDT.methods.balanceOf(this.address)
          .call({ from: this.address })
          .then((value: any) => +value / 1000000),
        approve: (amount) => contractUSDT.methods.approve(this.configs['SMC_MAIN_ADDRESS'], NumberUtils.cryptoConvertToStringAmount(amount, 6))
          .send({ from: this.address }),
        allowance: () => contractUSDT.methods.allowance(this.address, this.configs['SMC_MAIN_ADDRESS'])
          .call({ from: this.address })
          .then((value: any) => +value / 1000000),
      }

      this.contractStaking = new this.web3.eth.Contract(this.configs['SMC_STAKING_ABI'], this.configs['SMC_STAKING_ADDRESS'], { from: this.address }) as any;
      ...
      if (this.chainId !== this.configs.SMC_NETWORK_CHAIN_ID) throw Error(`You must connect to ${this.configs.SMC_NETWORK_NAME} network`)
  ...
  static async fetchPools(store: Store) {
    try {
      ...
      const pools = await Promise.all(new Array(6)
        .fill(0)
        .map(async (_, key) => ({
          pid: key,
      ...
      store.dispatch({ type: SET_SMC_POOLS_ERROR, status: ESMCStatus.ERROR, error: errorMesage });
    }
  }
  ...
  static async call(options: {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (typeof func !== 'function') reject(AppService.throwError({ dev: `Cannot find function contract.methods.${options.method} ` }));
  ...
  static async send(options: {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const gas = await func(...args).estimateGas()
      ...
      func(...args)
        ...
        .on('transactionHash', function (transactionHashReceived: any) {
          ...
          interval = setIntervalAsync(async () => {
            const transactionReceipt = await SmcService.web3.eth.getTransactionReceipt(transactionHashReceived) as Transaction;
  ...
  static async requestApprove(contracts: {
    fromAddress: string,
    toContract: Contract,
  }, amount: number, decimals = 18) {
    // Check allowance
    const allowance = await SmcService.call({
      contract: contracts.toContract,
      method: 'allowance'
    }, this.address, contracts.fromAddress)
    ...
    if (allowance < amount) {
      ...
      await this.send(
        ...
        contracts.fromAddress, "0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"
      )
        .catch((error) => {
          console.error(error);
  ...
  static transactionErrorAlert(error: any, message?: string) {
    ...
    return AppService.createErrNoti(msg, error && error.transactionHash && {
      buttons: [
        {
          label: 'View Transaction',
          onClick: () => window.open(SmcService.renderLinkTransaction(error.transactionHash))
        }
      ]
    })
  }

  static transactionSuccessAlert(transaction: Transaction, message: string): Transaction {
    AppService.createSuccessNoti(message, {
      buttons: [
        {
          label: 'View Transaction',
          onClick: () => window.open(SmcService.renderLinkTransaction(transaction.transactionHash))
        }
      ]
    });
